...
class Ethnicity(object):
    ...
    def __init__(self):
        ...
        self.ETHNICITIES = set(
            """indian japanese greek arabic turkish iranian 
        ...
        assert self.ETHNICITIES <= set(
            os.listdir(self.DATADIR)
        ), "** error ** data is missing for some ethnicities!"
        ...
        self.QUOTES = {
            "russian": {"last_names": 1, "first_names": 2},
            ...
            "south-slavic": {"last_names": 1, "first_names": 2},
            "italian": {"last_names": 1, "first_names": 2},
            "polish": {"last_names": 2, "first_names": 1},
            "anglo-saxon": {"last_names": 2, "first_names": 1},
            ...
            "hispanic": {"last_names": 1, "first_names": 2},
    ...
    def __readtext(self, file):
        ...
        return {
            unidecode(l.strip())
            for l in open(os.path.join(self.DATADIR, file), "r").readlines()
            if l.strip()
        }
    ...
    def make_dicts(self, refresh=True):
            ...
            if "full_names" in d[e]:
                for name in ["first_names", "last_names"]:
                    if name in d[e]:
                        ...
                        lst = [
                            full_name.split()[pick_idx]
                            for full_name in d[e]["full_names"]
                        ]
        ...
        d = (
            pd.read_csv(os.path.join(self.DATADIR, "race_", "race_.csv"))
            .iloc[:, [0, 5, 6, 7, 8, 9, 10]]
            .dropna()
        )
        d["name"] = d["name"].str.lower()
        d.iloc[:, 1:] = d.iloc[:, 1:].applymap(
            lambda x: 0 if not str(x).replace(".", "").isdigit() else x
        )
        ...
        self.__writejson(
            self.ETHNIC_NAMES_U, os.path.join(self.TEMP_DIR, "dic_names_.json")
        )
        self.__writejson(
            self.ETHNIC_SURNAMES_U, os.path.join(self.TEMP_DIR, "dic_surnames_.json")
        )
        self.__writejson(
            self.ETHNIC_ENDINGS_U, os.path.join(self.TEMP_DIR, "dic_sur_endings_.json")
        )

        json.dump(
            self.RACE_DIC, open(os.path.join(self.TEMP_DIR, "dic_race_.json"), "w")
        )
    ...
    def _get(self, st):
        ...
        surname_ending_ethnicities = (
            self.search_surname_endings(_surname) if _surname else set()
        )
    ...
    def get(self, s):
        ...
        # and in case s is a list or names
            ...
            return pd.DataFrame(
                {
                    "Name": [
                        _.title() if (_ and len(_) < self.MAXSTRLEN) else ""
                        for _ in [self._normalize(name) for name in s]
                    ],
                    "Ethnicity": [
                        "|".join(list(_)) if _ else "---" for _ in ethnicities_
                    ],
                }
            )
